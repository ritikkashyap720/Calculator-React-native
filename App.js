...
export default function App() {
  ...
  const getValue = (e) => {
    ...
    // console.log(getValue.slice(getValue.length-1))
    setvalue(getValue)
    console.log(e)
    var getValueString = getValue.toString()

    for (let index1 = 0; index1 < operatorArray.length; index1++) {
      if (getValueString.endsWith(operatorArray[index1])) {
        for (let index = 0; index < operatorArray.length; index++) {
          if (getValueString.slice(0, -1).endsWith(operatorArray[index])) {
            var newValue = getValue.slice(0, -1).slice(0, -1)
            setvalue(newValue + operatorArray[index1])
          }
        }
      }
    }
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={styles.container}>
        <View style={styles.display}>
          ...
          {isCalculatedValue && <Text style={{ color: "white", fontSize: 30, opacity: 0.5 }}>= {calculatedValue}</Text>}
        </View>
